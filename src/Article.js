...
const Enums = Object.freeze({
  ...
  type: ['Review Article', 'Blog', 'Original Research', 'Magazine Article'],
  /** @type {Array} */
  subject: [
    'Biology',
    'Chemistry',
...
module.exports = class Article {
  ...
  setAuthor(emails = '', authors = []) {
    emails = emails.split(',')
    this.authors = removeDuplicatesProperties(
      authors.filter(j => emails.includes(j.email)),
      'email'
    )
  }
  ...
  setEditor(emails = '', editors = []) {
    emails = emails.split(',')
    this.editors = removeDuplicatesProperties(
      editors.filter(j => emails.includes(j.email)),
      'email'
    )
  }
  ...
  assignProperties(properties) {
    ...
    Object.keys(properties)
      .filter(p =>
        Enums[p]
          ? p == 'status'
            ? Enums[p].some(s => s.name == properties[p])
            : Enums[p].includes(properties[p]) && Enums[p] != Strings.IMMUTABLE
          : true
      )
   ...
   */
  isFullSubmission() {
    const minimumProps = ['title', 'subject', 'type', 'summary', 'reason']
