...
module.exports = class Router {
  ...
  setOptions(e) {
    ...
    if (e.path) this.path = e.path.split('/').slice(1)
    else {
  ...
  async route() {
    ...
    if (!selectedContext) {
      ...
      return new ErrorResponse(
        `No such action ${action} exists for context ${context}`
      )
    }
    ...
    if (
      auth.authenticationLevel == AuthenticationLevels.UNAUTHORISED ||
    ...
    if (track !== null) {
      ...
      } else return new ErrorResponse(Errors.UNAUTHORISED_ACCESS)
    } else {
      return new ErrorResponse(Errors.INTERNAL_ERROR)
    }
  }
