...
class Authentication {
  ...
  async getUsersFromSheet() {
    let matchingUsers = await SheetUtils.getMatchingRowsFromSheet(
      this.baseAuthSheetName,
      {
        email: this.email
      }
    )
  ...
  async sendAuthEmail() {
    // Check state
    if (!this.states.emailIsAllowed) return new EmailSendFailure()
  ...
  createKeySheetRow(data) {
    if (!data.key)
      throw new Error(`Missing Key @ ${this.createKeySheetRow.name}`)
    if (!data.email)
      throw new Error(`Missing Email @ ${this.createKeySheetRow.name}`)

    data.keepLoggedIn = data.keepLoggedIn || false
    data.level = data.level || AuthenticationLevels.UNAUTHORISED

    return [new Date(), data.email, data.key, data.level, data.keepLoggedIn]
  }
  ...
  createAuthTokenSheetRow(data) {
    if (!data.token || !data.email)
      throw new Error(`Missing data @ ${this.createAuthTokenSheetRow.name}`)

    data.keepLoggedIn = data.keepLoggedIn || false

    return [new Date(), data.email, data.token, data.level, data.keepLoggedIn]
  }
...
class AuthenticationResource {
  ...
  constructor(data) {
    ...
    this.authenticationLevel =
      data.authenticationLevel || AuthenticationLevels.UNAUTHORISED
