...
class SheetUtils {
  ...
  static async getSheetsAsJSON(sheets, enableCache) {
    if (!(sheets instanceof Array))
      throw new TypeError('Missing argument @ getSheetAsArray')
  ...
  static async getMatchingRowsFromSheet(sheetName, properties) {
    if (!properties || !sheetName)
      throw new Error('Missing arguments @ getMatchingRowsFromSheet')
  ...
  static async removeMatchingRowFromSheet(sheetName, properties) {
    if (!sheetName || !properties)
      throw new TypeError(
        `Missing arguments. Was given sheetName: ${sheetName}, properties: ${uniqueValue}`
      )
  ...
  static async updateMatchingRow(properties, row, sheetName) {
    if (!properties || !row || !sheetName)
      throw new Error('Missing arguments @ SheetUtils.updateMatchingRow')
    const data = await this.getSheetAsJSON(sheetName)
  ...
  static async getMatchingRowRange(identifiers, sheetName, offset = 0) {
    if (!identifiers || !sheetName)
      throw new Error('Missing parameters @ getMatchingRowRange')
  ...
  static async pushRowsToSheet(rows, sheetName) {
    if (!(rows instanceof Array)) return new Error('Rows must be an array')
